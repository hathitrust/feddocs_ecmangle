...
require 'ecmangle'
module ECMangle
  class MonthlyLaborReview < ECMangle::DefaultMangler
    ...
    def explode(ec, _src = nil)
      ...
      if ec['start_number']
        ...
        sm = ECMangle::MONTHS.index(ECMangle.lookup_month(ec['start_month']))
        em = ECMangle::MONTHS.index(ECMangle.lookup_month(ec['end_month']))
        (sm..em).each do |n|
          ecn = ec.clone
          ecn['number'] = n + 1
          ecn['month'] = ECMangle::MONTHS[n]
          ecs << ecn
        end
      else
        ecs << ec
    ...
    def canonicalize(ec)
      ...
      if ec['volume']
        ...
        if ec['number']
          ...
          ec['month'] ||= ECMangle::MONTHS[ec['number'].to_i - 1]
        end
        canon += ", Year:#{ec['year']}" if ec['year']
        canon += ", Month:#{ECMangle.lookup_month(ec['month'])}" if ec['month']
