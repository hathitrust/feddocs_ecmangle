...
module ECMangle
  ...
  class ForeignRelations < ECMangle::DefaultMangler
    def initialize
      @title = 'Foreign Relations'
      @sudoc_stems = ['S 1.1:']
      # @ocns = [10648533, 1768670]
      super
    ...
    def parse_ec(ec_string)
      ...
      ec_string = ec_string.gsub(/ COPY$/, '')
                           .gsub(/ ?=.*/, '')
                           .gsub(/#{div}FICHE \d+(-\d+)?$/, '')
                           .gsub(/#{div}MF\.? SUP\.?$/, '')
                           .chomp
                           .gsub(/^KZ233 . U55 /, '') # some junk in the front
                           .gsub(/^V\. \/V/, 'V') # some junk in the front
                           .gsub(/SUP\.?([^P])?/, 'SUPPLEMENT\1') # expand some stuff
                           .gsub(/CONF\.?([^E])?/, 'CONFERENCE\1') # expand some stuff
                           .gsub(/#{div}SUPPLEMENT$/, '') # just telling us supplement is useless
      ...
      # fix some manglings
      ec_string = ec_string.gsub(/(\d{2,4})V/, '\1 V').gsub(/(\d)PT/, '\1 PT')
