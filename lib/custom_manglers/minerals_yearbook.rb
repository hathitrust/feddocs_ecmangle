...
require 'ecmangle'
module ECMangle
  class MineralsYearbook < ECMangle::DefaultMangler
    def initialize
      @title = 'Minerals Yearbook'
      @ocns = [1_847_412, 228_509_857, 48_997_937]
      super
    ...
    def parse_ec(ec_string)
      ...
      # useless junk
      ec_string = ec_string.sub(/^TN23 \. U612 /, '')
      ...
      unless matchdata.nil?
        ...
        # kill the zero fills
        if ec['volume']
          ec['volume'] = ec['volume'].sub(/^0+/, '')
        elsif ec['start_volume']
          ec['start_volume'] = ec['start_volume'].sub(/^0+/, '')
          ec['end_volume'] = ec['end_volume'].sub(/^0+/, '')
        end
    ...
    def normalize_description(desc)
      ...
      desc = desc.sub(/^AREA\s?RE?P(OR)?TS:?\s?/, '') if desc !~ /^AREA REPORTS$/

      desc = desc.sub(/^INTL:/, '')
                 .sub(/U\.?\s?S\.?\s?S\.?\s?R\.?\s?/, 'USSR')
                 .sub(/AFRICA\/\s?(THE\s)?MID/, 'AFRICA AND THE MID')
                 .sub(/ASIA[\/:]\s?(THE\s)?PAC.*/, 'ASIA AND THE PACIFIC')
                 .sub(/^ASIA[^\/]*/, 'ASIA AND THE PACIFIC')
                 .sub(/EUROPE[\/:]\s?CEN/, 'EUROPE AND CEN')
                 .sub(/EUROPE\sCEN/, 'EUROPE AND CEN')
                 .sub(/EUROPE[\/:]\s?USSR/, 'EUROPE AND THE USSR')
                 .sub(/LATIN\sAMERICA[\/:]\s?CANADA/, 'LATIN AMERICA AND CANADA')
                 .sub(/EUROPE AND CE?N?(TRA)?\.? EUR/, 'EUROPE AND CENTRAL EURASIA')
                 .sub(/METALS[\/:]\s?MI/, 'METALS AND MI')
                 .sub(/MID\.?\s/, 'MIDDLE ')
                 .sub(/AND\sMIDDLE/, 'AND THE MIDDLE')
                 .sub(/MIDDLE$/, 'MIDDLE EAST')
                 .sub(/MIN[\.\s]\s? IND/, 'MINERAL IND')
                 .sub(/INDUST\./, 'INDUSTRIES')
                 .sub(/^AFRICA MIDDLE EAST$/, 'AFRICA AND THE MIDDLE EAST')
      # seriously people
      desc = desc.sub(/^LATIN$/, 'LATIN AMERICA AND CANADA')
                 .sub(/^WORLD ECONO$/, 'WORLD ECONOMY')
                 .sub(/^EUROPE AND( CE(NTRAL?)?)?$/, 'EUROPE AND CENTRAL EURASIA')
                 .sub(/^EUROPE AND CENTRAL E$/, 'EUROPE AND CENTRAL EURASIA')
      desc
    end
