...
module ECMangle
  ...
  class StatisticalAbstract < ECMangle::DefaultMangler
    ...
    def parse_ec(ec_string)
      ...
      ec_string = ec_string.gsub(/REEL \d+.* P77-/, '')
                           .gsub(/^A V\./, 'V.')
                           .gsub(/^: /, '')
                           .gsub(/^C\. \d+ V/, 'V')
                           .gsub(/^C\. \d+ ?/, '')
                           .gsub(/ \)/, ')') # space before trailing ) is always a typo
                           .gsub(/[,: ]$/, '') # trailing junk
                           .gsub(/ ?= ?[0-9]+.*/, '') # remove unnecessary crap
                           .gsub(/ C(OP)?\. \d$/, '') # remove useless 'copy' information
                           .gsub(/ - WD/, '') # we don't care about withdrawn status
      ...
      ec_string = ec_string.gsub(/(?<y>\d{4}) \(?\k<y>\)?/, '\k<y>')
